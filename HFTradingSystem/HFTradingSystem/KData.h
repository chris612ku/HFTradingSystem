...
extern const string KbarDirectory; // = HFTradingSystemDirectoryPrefix + "KBar/";
...
//operations;构造函数，初始化k线数据
	...
	{
		cerr << endl << endl;
		cerr << "InstrumentID: " << m_sInstrumentID
			<< ", Index:" << m_nIndex
			<< ", ActionDay: " << m_sActionDay
			<< ", BeginTime: " << m_sBeginTime
			<< ", EndTime: " << m_sEndTime
			<< ", OpenPrice: " << m_fOpen
			<< ", HighestPrice: " << m_fHighest
			<< ", LowestPrice: " << m_fLowest
			<< ", ClosePrice: " << m_fClose
			<< ", Volume: " << m_nVolume
	...
	int SaveKBar(std::string strPath)
	{
		...
		try {
			char sqlStatement[400] = { '\0' };
			sprintf_s(sqlStatement, sizeof(sqlStatement),
				"INSERT INTO %s (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)\
				 VALUES('%s', '%s', '%s', '%s', '%s', '%lf', '%lf', '%lf', '%lf', '%lf', '%d', '%d', '%d'); ",
				marketdataTable.c_str(), marketdata_instrumentID.c_str(), marketdata_actionDay.c_str(), marketdata_tradingDay.c_str(), 
				marketdata_beginTime.c_str(), marketdata_endTime.c_str(), marketdata_fOpen.c_str(), marketdata_fHighest.c_str(), 
				marketdata_fLowest.c_str(), marketdata_fClose.c_str(), marketdata_fAverage.c_str(), marketdata_nVolume.c_str(), 
				marketdata_nPosition.c_str(), marketdata_nPositionDiff.c_str(), this->m_sInstrumentID, this->m_sActionDay,
				this->m_sTradingDay, this->m_sBeginTime, this->m_sEndTime, this->m_fOpen, this->m_fHighest, this->m_fLowest, this->m_fClose, 
				this->m_fAverage, this->m_nVolume, this->m_nPosition, this->m_nPositionDiff);
...
public:
	...
	CKBarSeries * CreateSeries(const char * pszName,BarType barType,int nInterval,int volumeMultiply,int nTotal){
		...
		if (its != pMap->end())
		{
			...
			ReadKBarSeries(KbarDirectory+its->first,its->second);
		}
